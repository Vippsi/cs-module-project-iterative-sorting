...
def bubble_sort(arr):
    ...
    for i in range(len(arr)-1):
        for j in range(0, len(arr)-i-1): # len(arr)-i-1 is creating the bounds example: if arr length if 5 and we're current on index 2 then the range would be 5-2-1 so we would have range(0-2)
            if arr[j] > arr[j + 1]:
